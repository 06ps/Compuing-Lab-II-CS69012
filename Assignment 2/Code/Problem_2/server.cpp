...
server performs select operation and handles client request accordingly. 
*/
        ...
        for(int x=0; x<FD_SETSIZE; x++)
        {
            if(FD_ISSET(x, &temp_fds))
            {
                if(x == socket_fd)
                {
                    ...
                    if(time_start[newsocket_fd] == 0)
                    {
                        time_start[newsocket_fd] = time(&begin);
                    }
                        ...
                        if(is_valid(expression))
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    time(&end);
                                    //cout << time_start[x] <<" ts " << end <<endl;
                                    double time = 1.0 * (end - time_start[x]);
                                    string file_ip = "Client ID: " + arr[x] + " , "+"  Infix expression: " + buffer + " , " +"  Postfix expression: " + infix_2_postfix + " , " + "  Answer: " + result + " , " + "  Time elapsed: " + to_string(time)+"sec";
                                    ofstream out;
